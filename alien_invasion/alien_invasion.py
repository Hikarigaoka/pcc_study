...
class AlienInvasion:
    ...
    def _change_fleet_direction(self):
        ...
        for alien in self.aliens.sprites():
            alien.initial_y += self.settings.fleet_drop_speed
    ...
    def _create_alien(self, alien_number, row_number):
        """외계인을 만들고 줄에 배치합니다"""
        alien = Alien(self, self.wave_number)
        alien_width, alien_height = alien.rect.size
    ...
    def _ship_hit(self):
        """우주선이 피격되었을 때 처리"""
        print("Ship hit!!!")
        # 게임 종료 또는 생명력 감소 로직 (여기서는 간단히 종료)
        sys.exit()

    def _check_alien_fire(self):
        """외계인이 무작위로 탄환을 발사합니다"""
        for alien in self.aliens.sprites():
            if random.random() < 0.0005: # 발사 확률
    ...
    def _update_screen(self):
        ...
        # 별 그리기
        for star in self.stars:
            pygame.draw.circle(self.screen, (200, 200, 200), star, 2)
